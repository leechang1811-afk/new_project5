/**
 * 롤모델별 루틴은 공개 인터뷰·전기·보도 등에서 자주 인용되는 습관을 바탕으로 짧게 옮겼습니다.
 * (의학·법률 자문 아님 — 앱 내 동기 부여용)
 */

export const CELEBRITIES = {
  jobs: {
    name: '스티브 잡스',
    oneLine: '핵심 하나에 몰입',
    mediaNote: '※ 본질에 집중하는 습관을 전기·인터뷰에서 자주 인용되는 흐름을 바탕으로 옮겼어요.',
    routines: [
      '아침에 오늘 가장 중요한 문제 1개를 먼저 적기',
      '오전에 방해 없는 몰입 시간으로 핵심 작업 진행하기',
      '점심 전후에 짧은 산책 또는 워킹 미팅으로 생각 정리하기',
      '오후에 제품 관점에서 사용자 경험 1가지를 재점검하기',
      '저녁에 불필요한 업무·기능을 덜어내는 체크리스트 정리',
    ],
  },
  musk: {
    name: '일론 머스크',
    oneLine: '실행과 문제 해결',
    mediaNote: '※ 시간 단위 스케줄·현장 중심 실행을 강조한 보도·인터뷰 흐름을 바탕으로 구성했어요.',
    routines: [
      '아침에 중요한 이슈 메일·메신저를 빠르게 분류하기',
      '오전에 가장 어려운 문제를 해결하는 블록 작업 진행',
      '점심은 짧게 하고 업무 흐름을 끊지 않기',
      '오후에 현장/엔지니어링 팀과 진행 상황을 직접 점검하기',
      '저녁에 기술 문서·설계안을 검토하고 다음 액션 정리',
    ],
  },
  bezos: {
    name: '제프 베조스',
    oneLine: '고품질 의사결정',
    mediaNote: '※ “적은 수의 고품질 결정” 원칙과 메모 중심 회의를 정리해 하루 루틴으로 옮겼어요.',
    routines: [
      '아침에 컨디션을 먼저 점검하고 일정 강도를 조정하기',
      '오전에 고난도 의사결정 1~2개를 우선 처리하기',
      '회의 전 6페이지 내외 메모를 읽고 핵심 질문 정리',
      '오후에 장기 지표와 고객 관점 전략을 재확인하기',
      '저녁에 회복·휴식으로 다음 날 판단력을 유지하기',
    ],
  },
  buffett: {
    name: '워런 버핏',
    oneLine: '독서와 장기 관점',
    mediaNote: '※ 하루 대부분을 읽고 생각하는 투자 스타일을 루틴 형태로 정리했어요.',
    routines: [
      '아침에 경제/기업 기사와 주주서한을 읽으며 시작하기',
      '오전에 투자 아이디어를 숫자와 가정으로 검증하기',
      '불필요한 일정은 줄이고 생각 시간을 확보하기',
      '오후에 장기 관점에서 리스크와 가치 재평가하기',
      '저녁에 독서 메모를 남기고 다음 읽을 자료를 정하기',
    ],
  },
  gates: {
    name: '빌 게이츠',
    oneLine: '깊은 사고와 학습',
    mediaNote: '※ Think Week와 독서·메모 습관을 기반으로 사고 확장 흐름을 정리했어요.',
    routines: [
      '아침에 책·리포트로 새로운 주제 1개 학습하기',
      '오전에 깊은 사고가 필요한 문제를 단독으로 검토하기',
      '점심 전후에 메모를 정리해 질문 목록으로 전환하기',
      '오후에 전문가 대화/미팅으로 가설을 점검하기',
      '저녁에 배운 내용을 짧은 노트로 구조화하기',
    ],
  },
  zuckerberg: {
    name: '마크 저커버그',
    oneLine: '제품과 실행',
    mediaNote: '※ 제품 중심 사고와 빠른 실행·운동 습관을 아침·저녁 루틴으로 재구성했어요.',
    routines: [
      '아침에 결정을 단순화해 바로 일과를 시작하기',
      '오전에 제품·개발 우선순위를 기준으로 실행하기',
      '점심 전후 운동으로 에너지 레벨을 재정비하기',
      '오후에 팀과 핵심 지표·실험 결과를 빠르게 리뷰하기',
      '저녁에 기술 트렌드/AI 동향을 짧게 학습하기',
    ],
  },
  tim_cook: {
    name: '팀 쿡',
    oneLine: '아침 정리 루틴',
    mediaNote: '※ 이른 시간에 일정을 정리하고 우선순위를 세운다는 보도를 바탕으로 구성했어요.',
    routines: [
      '이른 아침에 주요 메일과 팀 이슈를 먼저 확인하기',
      '오늘 최우선 과제 1개를 정하고 일정에 고정하기',
      '회의는 핵심 안건 중심으로 짧고 명확하게 운영하기',
      '하루 중간에 고객/제품 관점으로 우선순위를 재정렬하기',
      '퇴근 전 다음 날 첫 의사결정 포인트를 미리 기록하기',
    ],
  },
  oprah: {
    name: '오프라 윈프리',
    oneLine: '감사와 마음 챙기기',
    mediaNote: '※ 감사 일기·명상을 공개적으로 이야기한 인터뷰·쇼를 바탕으로 구성했어요.',
    routines: [
      '감사한 일 3가지 적기',
      '호흡·명상 5분',
      '고마운 사람에게 메시지·통화 1번',
      '인터뷰·강연·책에서 배운 문장 1개 기록',
      '하루 마무리 전 감정 상태를 짧게 점검',
    ],
  },
  michelle_obama: {
    name: '미셸 오바마',
    oneLine: '이른 운동과 균형',
    mediaNote: '※ 새벽 운동 습관을 본인이 인터뷰에서 언급한 내용을 바탕으로 구성했어요.',
    routines: [
      '가벼운 운동·걷기 20분',
      '하루 목표를 짧게 한 줄로 쓰기',
      '가족과 식사·대화 한 번',
      '중요한 일정 전에 준비 메모 1개 작성',
      '저녁에 오늘 에너지 사용을 간단히 돌아보기',
    ],
  },
  obama: {
    name: '버락 오바마',
    oneLine: '운동과 가족 시간',
    mediaNote: '※ 아침 운동·가족 식사를 중시한다는 보도·인터뷰를 바탕으로 구성했어요.',
    routines: [
      '운동·산책 20분',
      '당일 중요한 일정·일 1개만 고르기',
      '가족과 이야기 나누기',
      '결정이 필요한 이슈를 10분 먼저 읽고 정리',
      '하루 끝에 다음 날 우선순위 1개 확정',
    ],
  },
  beyonce: {
    name: '비욘세',
    oneLine: '연습과 컨디션',
    mediaNote: '※ 리허설·훈련 강도에 대한 보도·다큐 흐름을 바탕으로 구성했어요.',
    routines: [
      '보컬·몸풀기 10분',
      '집중 연습·블록 25분',
      '오늘 몸 상태 한 줄 기록',
      '퍼포먼스 영상 1개 복기하며 디테일 체크',
      '수면·회복 시간을 먼저 일정에 고정',
    ],
  },
  taylor_swift: {
    name: '테일러 스위프트',
    oneLine: '글과 감정 기록',
    mediaNote: '※ 작사·일기처럼 쓰는 습관이 인터뷰에서 자주 언급돼요.',
    routines: [
      '아이디어·가사 메모 10분',
      '조용히 글쓰기·연습 20분',
      '오늘 감정 한 줄 기록',
      '떠오른 멜로디를 음성메모로 즉시 저장',
      '하루 한 번 창작물 초안 수정 시간 확보',
    ],
  },
  messi: {
    name: '리오넬 메시',
    oneLine: '기본기와 회복',
    mediaNote: '※ 훈련·회복 루틴이 스포츠 매체에서 자주 다뤄져요.',
    routines: [
      '아침에 가벼운 몸풀기 하기',
      '오전에 기술 훈련하기',
      '점심에 식단 관리하기',
      '오후에 팀 훈련 및 전술 연습하기',
      '저녁에 회복 및 휴식하기',
    ],
  },
  son: {
    name: '손흥민',
    oneLine: '기본기 반복',
    mediaNote: '※ 추가 슈팅 연습 등 훈련 태도가 국내외 스포츠 보도에서 반복 인용돼요.',
    routines: [
      '몸풀기·스트레칭 10분',
      '슈팅·첫 터치 연습 20분',
      '경기·훈련 복기 한 줄 쓰기',
      '짧은 스프린트·민첩성 훈련 10분',
      '훈련 후 회복 루틴(쿨다운·수분) 챙기기',
    ],
  },
  iu: {
    name: '아이유',
    oneLine: '꾸준한 창작',
    mediaNote: '※ 작업실 루틴·자기 점검을 음악·방송 인터뷰에서 이야기한 흐름을 바탕으로 구성했어요.',
    routines: [
      '보컬·호흡 워밍업 10분',
      '조용히 작업·연습 25분',
      '오늘 느낀 점 짧게 기록',
      '좋아하는 음악 1곡 분석하며 듣기',
      '잠들기 전 다음 작업 첫 문장 미리 정하기',
    ],
  },
  serena: {
    name: '세리나 윌리엄스',
    oneLine: '집중 훈련과 회복',
    mediaNote: '※ 코트·보강 훈련과 회복·식단 관리에 대한 보도를 바탕으로 구성했어요.',
    routines: [
      '아침에 스트레칭 및 준비 운동하기',
      '오전에 테니스 훈련하기',
      '점심에 식단 관리하기',
      '오후에 웨이트 및 보강 훈련하기',
      '저녁에 회복 관리하기',
    ],
  },
  churchill: {
    name: '윈스턴 처칠',
    oneLine: '야행성과 낮잠',
    mediaNote: '※ 침대 업무·낮잠·야간 작업 패턴을 전기·역사 자료에서 가져와 재구성했어요.',
    routines: [
      '아침에 침대에서 업무 시작하기',
      '오전에 문서 및 보고 검토하기',
      '점심 후 휴식하기',
      '오후에 낮잠 후 업무 재개하기',
      '저녁에 연설 작성 및 집중 업무 수행하기',
    ],
  },
  einstein: {
    name: '알베르트 아인슈타인',
    oneLine: '산책과 깊은 사고',
    mediaNote: '※ 산책·수면·음악을 통한 사색 습관을 전기에서 인용해 하루 루틴으로 정리했어요.',
    routines: [
      '아침에 여유롭게 시작하기',
      '오전에 깊은 사고 및 연구하기',
      '점심 후 산책하기',
      '오후에 연구 및 정리하기',
      '저녁에 음악 또는 휴식하기',
    ],
  },

  // —— 한국 · 연예인 (방송·인터뷰·보도에서 자주 인용되는 준비·습관 흐름)
  yoo_jae_suk: {
    name: '유재석',
    oneLine: '철저한 준비와 맞춤법',
    mediaNote: '※ 예능·인터뷰에서 반복 언급되는 사전 준비·대본 점검 습관을 바탕으로 구성했어요.',
    routines: [
      '오늘 할 일·대본·메모를 10분만 정리',
      '중요한 말 한마디를 미리 한 줄로 쓰기',
      '끝난 뒤 “다음엔 이렇게” 한 줄 복기',
      '촬영·회의 전 상대 관점 질문 1개 준비',
      '일정 사이 짧은 휴식으로 컨디션 관리',
    ],
  },
  lim_young_woong: {
    name: '임영웅',
    oneLine: '발성과 마음가짐',
    mediaNote: '※ 방송·기사에서 언급되는 연습·무대 전 마음 다잡기 흐름을 바탕으로 구성했어요.',
    routines: [
      '호흡·발성 워밍업 10분',
      '집중 연습·녹음 블록 20분',
      '오늘 감사·다짐 한 줄',
      '무대 영상 1개 보고 표현 포인트 체크',
      '목 상태·컨디션 기록 후 회복 루틴 적용',
    ],
  },
  gong_yoo: {
    name: '공유',
    oneLine: '운동과 독서',
    mediaNote: '※ 인터뷰에서 운동·독서 습관을 이야기한 내용을 바탕으로 구성했어요.',
    routines: [
      '가벼운 운동·걷기 20분',
      '책·시나리오 20분 읽기',
      '오늘 인상 깊은 대사·문장 한 줄',
      '촬영 전 장면 의도 1줄 정리',
      '하루 끝에 집중력이 높았던 시간대 기록',
    ],
  },
  son_ye_jin: {
    name: '손예진',
    oneLine: '촬영 전 준비와 케어',
    mediaNote: '※ 작품 준비·자기관리에 대해 인터뷰에서 언급한 흐름을 바탕으로 구성했어요.',
    routines: [
      '오늘 집중할 장면·감정 한 줄 적기',
      '대본·감정 메모 15분',
      '스트레칭·호흡으로 몸 풀기 10분',
      '촬영 전 발성·표정 워밍업 5분',
      '촬영 후 장면 피드백 1개만 정리',
    ],
  },

  // —— 한국 · 기업 CEO (언론·인터뷰·연설 등)
  kim_bong_jin: {
    name: '김봉진',
    oneLine: '빠른 실행과 고객',
    mediaNote: '※ 스타트업·서비스 관련 인터뷰·강연에서 자주 나오는 실행·피드백 흐름을 바탕으로 구성했어요.',
    routines: [
      '오늘 풀 고객 불편 1개를 문장으로 정의하기',
      '짧은 주기로 시안/프로토타입을 만들어 바로 검증하기',
      '사용자 리뷰와 문의를 읽고 반복 이슈를 태깅하기',
      '팀과 실행 결과를 공유하고 즉시 개선 액션 정하기',
      '하루 끝에 실패 1건과 학습 1건을 남기기',
    ],
  },
  kim_beom_seok: {
    name: '김범석 (Coupang)',
    oneLine: '고객 경험과 장기 투자',
    mediaNote: '※ 쿠팡의 운영·데이터·고객 중심 문화로 알려진 흐름을 바탕으로 구성했어요.',
    routines: [
      '고객 경험 지표를 먼저 보고 우선순위를 정하기',
      '단기 효율보다 장기 고객가치 기준으로 결정하기',
      '현장 운영 이슈를 직접 확인하고 병목을 파악하기',
      '실행 속도와 품질을 함께 보는 점검 루틴 운영',
      '하루 마감 전 고객 관점 개선점 1개를 기록하기',
    ],
  },
  bang_si_hyuk: {
    name: '방시혁',
    oneLine: '음악과 미팅',
    mediaNote: '※ 음악·기획 중심 일정이 보도·인터뷰에서 다뤄지는 흐름을 바탕으로 구성했어요.',
    routines: [
      '음악 제작 및 리뷰 반복',
      '아티스트와 지속 미팅',
      '글로벌 트렌드 분석',
      'IP 중심 전략 회의',
      '밤 시간 집중 작업',
    ],
  },
  lee_hae_jin: {
    name: '이해진',
    oneLine: '기술과 제품',
    mediaNote: '※ 기술·서비스 지향적 의사결정이 인터뷰·강연에서 강조된 흐름을 바탕으로 구성했어요.',
    routines: [
      '오늘 개선할 기능·이슈 1개만 고르기',
      '데이터·글 읽기 20분',
      '사용자 입장에서 질문 1개 적기',
      '기술 트렌드/AI 관련 기사 1개 요약',
      '제품 지표 1개를 정해 전일 대비 확인',
    ],
  },
  lee_jae_yong: {
    name: '이재용',
    oneLine: '일정과 결정',
    mediaNote: '※ 언론에서 보도되는 경영·일정 중심 하루 흐름을 일반화해 구성했어요.',
    routines: [
      '아침 일정·우선순위 15분 정리',
      '오늘 최종 결정 1개만 정하기',
      '퇴근 전 회고·내일 첫 일 1줄',
      '핵심 사업 리스크·기회 포인트 1개 점검',
      '현장/조직 관점 피드백 1건 확인',
    ],
  },

  // —— 한국 · 위인 (전기·교과서·기록 자료)
  king_sejong: {
    name: '세종대왕',
    oneLine: '학문과 기록 루틴',
    mediaNote: '※ 글과 기록, 백성을 생각하는 태도를 전기·교과서 자료에서 가져와 일상의 루틴으로 옮겼어요.',
    routines: [
      '아침에 오늘 배울 글·글자 1가지 적기',
      '백성·사람 입장에서 질문 1개 쓰기',
      '낮에는 배운 내용을 곰곰이 떠올려 보기',
      '저녁에 오늘 있었던 일을 짧게 정리하기',
      '하루를 한 줄로 요약해 기록하기',
    ],
  },
  yun_dong_ju: {
    name: '윤동주',
    oneLine: '시와 일기 루틴',
    mediaNote: '※ 시와 일기를 쓰던 습관을, 감정을 기록하는 하루 루틴으로 정리했어요.',
    routines: [
      '아침에 마음에 드는 시 한 구절 읽기',
      '하루 중 떠오른 감정을 짧게 메모하기',
      '저녁에 오늘 있었던 일을 시나 문장으로 적어 보기',
      '조용히 혼자 사색하는 시간 10분 갖기',
      '자기 전 오늘 마음을 한 줄로 정리하기',
    ],
  },
  kim_gu: {
    name: '김구',
    oneLine: '성찰 루틴',
    mediaNote: '※ 일기와 저서에서 드러나는 성찰·실천의 태도를, 일상 속 루틴으로 옮겼어요.',
    routines: [
      '아침에 오늘 나라·공동체를 위해 할 일 1줄 적기',
      '낮에 그 일을 향해 작은 행동 1개 실천하기',
      '하루 중 감사한 일 1가지 떠올리기',
      '저녁에 오늘의 선택을 돌아보기',
      '밤에 오늘을 반성하는 글 한 줄 쓰기',
    ],
  },

  // —— 한국 · 운동선수 (스포츠 보도·인터뷰)
  kim_yuna: {
    name: '김연아',
    oneLine: '연습과 표현',
    mediaNote: '※ 훈련·프로그램 준비가 스포츠 보도·다큐에서 반복 다뤄진 흐름을 바탕으로 구성했어요.',
    routines: [
      '스트레칭·코어 15분',
      '집중 연습 블록 25분',
      '오늘 동작·점수 느낌 한 줄',
      '영상으로 자세·라인 1개 체크',
      '훈련 후 회복(수면·영양) 계획 기록',
    ],
  },
  park_ji_sung: {
    name: '박지성',
    oneLine: '체력과 전술',
    mediaNote: '※ 훈련·자기관리가 해외·국내 스포츠 언론에서 자주 인용돼요.',
    routines: [
      '인터벌·런닝 또는 유산소 15분',
      '전술·포지션 메모 10분',
      '경기·훈련 복기 한 줄',
      '하체 밸런스·코어 보강 10분',
      '회복 루틴(스트레칭·수분·휴식) 고정',
    ],
  },
  kim_young_kyung: {
    name: '김연경',
    oneLine: '스파이크와 회복',
    mediaNote: '※ 배구 선수로서 훈련·부상 관리가 보도에서 자주 다뤄져요.',
    routines: [
      '몸풀기·어깨·발목 케어 10분',
      '서브·스파이크 폼 연습 20분',
      '컨디션·기분 한 줄 기록',
      '리시브·블로킹 기본 동작 10분 반복',
      '훈련 후 회복 강도(통증·피로도) 체크',
    ],
  },
  top5_global: {
    name: '전세계 상위 5% 루틴',
    oneLine: '검증된 고성과 데일리',
    mediaNote:
      '※ 생산성·행동과학 연구, 글로벌 리더 인터뷰, 고성과 습관 관련 보고서에서 반복적으로 공통 등장하는 패턴을 바탕으로 구성했어요.',
    routines: [
      '같은 시간대에 기상·취침하며 수면 리듬 고정하기',
      '아침에 운동 또는 빠른 걷기 20분으로 에너지 올리기',
      '하루 최우선 과제 1개를 정해 방해 없이 60~90분 집중하기',
      '매일 학습 시간 20분 확보해 지식/기술 업데이트하기',
      '저녁에 회고 5분: 성과 1개·개선점 1개·내일 첫 일 1개 기록',
    ],
  },
} as const;

const CELEBRITY_QUOTES: Partial<Record<keyof typeof CELEBRITIES, string[]>> = {
  musk: [
    '무언가가 충분히 중요하다면, 성공 가능성이 낮더라도 반드시 해야 한다.',
    '끈질기게 버티지 않으면 결국 포기하게 된다.',
    '위대한 회사를 만들고 싶다면 훌륭한 제품에 집중해야 한다.',
  ],
  jobs: [
    '늘 갈망하고, 늘 어리석게 도전하라.',
    '혁신은 리더와 추종자를 구분한다.',
    '단순함은 궁극의 정교함이다.',
  ],
  bezos: [
    '우리는 비전에는 집요하게, 실행 방식에는 유연하게 접근한다.',
    '고객에게 집착하라, 경쟁자에게가 아니라.',
    '장기적으로 생각하는 것이 가장 큰 경쟁력이다.',
  ],
  buffett: [
    '규칙 1: 절대 돈을 잃지 마라. 규칙 2: 규칙 1을 절대 잊지 마라.',
    '가격은 당신이 지불하는 것이고, 가치는 당신이 얻는 것이다.',
    '남들이 두려워할 때 욕심내고, 남들이 욕심낼 때 두려워하라.',
  ],
  gates: [
    '가장 불만이 많은 고객이야말로 가장 큰 배움의 원천이다.',
    '성공은 형편없는 교사다. 사람을 방심하게 만든다.',
    '큰 성과는 작은 개선이 쌓여서 만들어진다.',
  ],
  zuckerberg: [
    '가장 큰 위험은 아무 위험도 감수하지 않는 것이다.',
    '빠르게 움직이고 계속 깨뜨려라.',
    '완벽을 기다리기보다 먼저 실행하라.',
  ],
  kim_beom_seok: [
    '고객이 없으면 회사도 없다.',
    '고객 경험이 모든 의사결정의 기준이다.',
    '빠르게 실행하고 지속적으로 개선해야 한다.',
  ],
  bang_si_hyuk: [
    '좋은 콘텐츠는 결국 사람의 마음을 움직인다.',
    '스토리가 있는 콘텐츠가 오래 살아남는다.',
    '디테일이 완성도를 만든다.',
  ],
  serena: [
    '챔피언은 승리로 정의되는 것이 아니라, 넘어졌을 때 어떻게 다시 일어나는가로 정의된다.',
    '나는 항상 최고가 되기 위해 싸운다.',
    '자신을 믿지 않으면 아무것도 이룰 수 없다.',
  ],
  messi: [
    '꿈을 이루기 위해서는 끝까지 싸워야 한다.',
    '노력은 결코 배신하지 않는다.',
    '작은 차이가 큰 결과를 만든다.',
  ],
  einstein: [
    '상상력은 지식보다 더 중요하다.',
    '나는 특별히 똑똑한 것이 아니라, 단지 오래 생각할 뿐이다.',
    '문제는 그것을 만든 방식으로는 해결할 수 없다.',
  ],
  churchill: [
    '성공은 끝이 아니고, 실패는 치명적이지 않다. 중요한 것은 계속 나아가는 용기다.',
    '절대, 절대, 절대 포기하지 마라.',
    '위대함의 대가는 책임이다.',
  ],
  lee_jae_yong: [
    '위기 속에서도 미래를 위한 투자를 멈춰서는 안 된다.',
    '변화를 두려워하지 말고 준비해야 한다.',
    '기회는 준비된 사람에게 온다.',
  ],
  lee_hae_jin: [
    '기술은 사람을 위한 것이어야 한다.',
    '글로벌 시장을 기준으로 생각해야 한다.',
    '사용자 경험이 가장 중요하다.',
  ],
  kim_bong_jin: [
    '좋은 서비스는 디테일에서 완성된다.',
    '브랜드는 경험으로 만들어진다.',
    '작은 차이가 큰 감동을 만든다.',
  ],
  oprah: [
    '상처를 지혜로 바꿔라.',
    '자신의 삶에 책임을 져라.',
    '당신이 믿는 것이 결국 당신이 된다.',
  ],
  michelle_obama: [
    '그들이 낮게 가도, 우리는 더 높게 가야 한다.',
    '당신의 이야기를 두려워하지 마라.',
    '성공은 돈이 아니라 영향력이다.',
  ],
  obama: [
    '우리는 할 수 있다.',
    '변화는 우리가 만드는 것이다.',
    '희망은 두려움보다 강하다.',
  ],
  beyonce: [
    '힘은 주어지는 것이 아니다. 스스로 쟁취해야 한다.',
    '나는 스스로 기준을 만든다.',
    '완벽함은 반복에서 나온다.',
  ],
  taylor_swift: [
    '인생에서 무슨 일이 일어나든, 사람들에게 친절하게 대하라.',
    '당신의 이야기는 당신만이 쓸 수 있다.',
    '진짜 감정이 가장 강한 콘텐츠다.',
  ],
  tim_cook: [
    '가장 중요한 것은 정말 중요한 것에 집중하는 것이다.',
    '우리는 최고의 제품을 만들기 위해 존재한다.',
    '가치는 결코 타협할 수 없다.',
  ],
  king_sejong: [
    '백성을 사랑하는 마음이 곧 나라를 다스리는 근본이다.',
    '나라의 말이 중국과 달라 서로 통하지 아니하므로… (훈민정음 서문)',
    '백성이 쉽게 배우고 쓰도록 하는 것이 중요하다.',
  ],
  yun_dong_ju: [
    '하늘을 우러러 한 점 부끄럼이 없기를.',
    '오늘 밤에도 별이 바람에 스치운다.',
    '죽는 날까지 하늘을 우러러 부끄럼 없기를.',
  ],
  kim_gu: [
    '나는 우리나라가 세계에서 가장 아름다운 나라가 되기를 원한다.',
    '문화의 힘은 위대하다.',
    '한없이 가지고 싶은 것은 높은 문화의 힘이다.',
  ],
};

export type PresetCelebrityId = keyof typeof CELEBRITIES;
export type CelebrityId = PresetCelebrityId | 'other';

/**
 * 롤모델 선택 그리드 순서 (인트로 등)
 * 기업가 → 전세계 유명인 → 위인 → 연예인·가수(국내 연예·스포츠 등)
 */
export const PRESET_CELEBRITY_GROUPS: { label: string; ids: PresetCelebrityId[] }[] = [
  {
    label: '기업가',
    ids: [
      'jobs',
      'musk',
      'bezos',
      'buffett',
      'gates',
      'zuckerberg',
      'tim_cook',
      'kim_beom_seok',
      'kim_bong_jin',
      'bang_si_hyuk',
      'lee_hae_jin',
      'lee_jae_yong',
    ],
  },
  {
    label: '전세계 유명인',
    ids: [
      'oprah',
      'michelle_obama',
      'obama',
      'beyonce',
      'taylor_swift',
      'messi',
      'serena',
    ],
  },
  {
    label: '위인',
    ids: ['churchill', 'einstein', 'king_sejong', 'yun_dong_ju', 'kim_gu'],
  },
  {
    label: '연예인·가수',
    ids: [
      'yoo_jae_suk',
      'lim_young_woong',
      'gong_yoo',
      'son_ye_jin',
      'iu',
      'son',
      'kim_yuna',
      'park_ji_sung',
      'kim_young_kyung',
    ],
  },
];

export const PRESET_CELEBRITY_IDS: PresetCelebrityId[] = PRESET_CELEBRITY_GROUPS.flatMap((g) => g.ids);

export type CelebrityProfile = {
  name: string;
  oneLine: string;
  routines: string[];
  mediaNote: string;
  quotes?: string[];
};

export function getProfile(id: CelebrityId, customName: string): CelebrityProfile {
  if (id === 'other') {
    const name = customName.trim() || '내 롤모델';
    return {
      name,
      oneLine: '직접 정한 하루 한 걸음',
      routines: ['오늘 집중할 일 1개만 쓰기', '방해 없는 25분 실행', '오늘 느낀 점 1줄 기록'],
      mediaNote: '직접 입력한 롤모델이에요. 이름은 아래에 적어 주세요.',
    };
  }
  const c = CELEBRITIES[id];
  return {
    name: c.name,
    oneLine: c.oneLine,
    routines: [...c.routines].slice(0, 5),
    mediaNote: c.mediaNote,
    quotes: CELEBRITY_QUOTES[id] ?? [],
  };
}

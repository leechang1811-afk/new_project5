/**
 * 지구촌교회 공동체 성경읽기표 기반 (연대기순)
 * 출처: https://www.jiguchon.or.kr/bbs/board.php?bo_table=G07
 * PDF: https://www.jiguchon.or.kr/bbs/download2.php?bo_table=G01&wr_id=545
 */
...
export const BIBLE_BOOKS_ORDER: { id: string; name: string }[] = [
  ...
  { id: 'judges', name: '사사기' },
  { id: 'ruth', name: '룻기' },
  { id: '1samuel', name: '사무엘상' },
  { id: '2samuel', name: '사무엘하' },
  { id: '1kings', name: '열왕기상' },
  { id: '2kings', name: '열왕기하' },
  { id: '1chronicles', name: '역대상' },
  { id: '2chronicles', name: '역대하' },
  { id: 'ezra', name: '에스라' },
  ...
  { id: 'acts', name: '사도행전' },
  { id: 'romans', name: '로마서' },
  { id: '1corinthians', name: '고린도전서' },
  { id: '2corinthians', name: '고린도후서' },
  { id: 'galatians', name: '갈라디아서' },
  { id: 'ephesians', name: '에베소서' },
  { id: 'philippians', name: '빌립보서' },
  { id: 'colossians', name: '골로새서' },
  { id: '1thessalonians', name: '데살로니가전서' },
  { id: '2thessalonians', name: '데살로니가후서' },
  { id: '1timothy', name: '디모데전서' },
  { id: '2timothy', name: '디모데후서' },
  { id: 'titus', name: '디도서' },
  { id: 'philemon', name: '빌레몬서' },
  { id: 'hebrews', name: '히브리서' },
  { id: 'james', name: '야고보서' },
  { id: '1peter', name: '베드로전서' },
  { id: '2peter', name: '베드로후서' },
  { id: '1john', name: '요한일서' },
  { id: '2john', name: '요한이서' },
  { id: '3john', name: '요한삼서' },
  { id: 'jude', name: '유다서' },
...
/** 지구촌교회 일정 - 내용 반으로 (1~2장/일) */
const JIGUCHON_SAMPLE: ReadingItem[] = [
  { dayIndex: 1, book: '창세기', bookId: 'genesis', startCh: 1, endCh: 2, hasOverview: true },
  ...
  { dayIndex: 7, book: '창세기', bookId: 'genesis', startCh: 13, endCh: 14, hasOverview: true },
  ...
  { dayIndex: 26, book: '출애굽기', bookId: 'exodus', startCh: 1, endCh: 2, hasOverview: true },
...
/** 나머지 일정 생성 (1~2장/일 - 내용 반으로) */
function generateFullSchedule(): ReadingItem[] {
  const result = [...JIGUCHON_SAMPLE];
  let dayIndex = 31;
  ...
  let bookIdx = 1; // 출애굽기 (day 30에서 9-10장까지)
  let chStart = 11;
  ...
  while (dayIndex <= 730 && bookIdx < booksWithChapters.length) {
    ...
    result.push({
      ...
      startCh: chStart,
      endCh: chEnd,
      hasOverview: chStart === 1 && dayIndex > 1,
    });
...
/** 특정 책부터 시작하는 일정 반환 */
export function getScheduleFromBook(
  startBookId: string,
  customOrder?: string[]
): ReadingItem[] {
  ...
  const fromStart = FULL_SCHEDULE.filter(
    (r) => order.indexOf(r.bookId) >= startIdx
  );
  const beforeStart = FULL_SCHEDULE.filter(
    (r) => order.indexOf(r.bookId) < startIdx
  );
  return [...fromStart, ...beforeStart].map((r, i) => ({
    ...r,
    dayIndex: i + 1,
  }));
}

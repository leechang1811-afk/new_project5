...
export default function Home() {
  ...
  useEffect(() => {
    const allowed = ['/', '/mission', '/report', '/settings'];
  ...
  const missionHubSubtitle = useMemo(() => {
    ...
    if (!morningConfirmed) return '루틴 1개 정하기';
    return '완료 여부 선택 후 저장';
  }, [savedToday, morningConfirmed]);
      ...
      <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="w-full pt-3 sm:pt-4 pb-2 sm:pb-3">
        ...
        {isSettings && (
          <section className="mb-4 p-4 rounded-2xl border border-toss-border bg-white text-center">
            <p className="text-sm font-semibold text-toss-text mb-1">설정</p>
            <p className="text-xs text-toss-sub mb-3">롤모델·사진·초기화는 이 화면에서만 바꿔요.</p>
        ...
        {isMission && (
          <>
            <p className="text-xs text-toss-sub text-center mb-2 leading-relaxed">
              루틴을 정하고 완료를 저장하면 홈에 바로 반영돼요.
            </p>
              ...
              {!morningConfirmed ? (
                ...
                <>
                  ...
                  <button
                    ...
                    className="mt-4 w-full py-4 rounded-xl bg-toss-blue text-white text-base font-extrabold disabled:opacity-50"
                  >
